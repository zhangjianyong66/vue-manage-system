    ...
    <el-table class="mgb20" :style="{ width: '100%' }" border :data="tableData" :row-key="rowKey"
      ...
      <template v-for="item in columns" :key="item.prop">
        <el-table-column v-if="item.visible" :prop="item.prop" :label="item.label" :width="item.width"
          ...
          <!--图片类型-->
          <template #default="{ row, column, $index }" v-if="item.type === 'img'">
            <el-image :src="pathToUrl(row[item.prop])" :preview-src-list="[pathToUrl(row[item.prop])]"
                      class="colum-img" preview-teleported hide-on-click-modal>
            </el-image>
          </template>
          <template #default="{ row, column, $index }" v-if="!item.type">
            <slot :name="item.prop" :rows="row" :index="$index">
              ...
              <!--普通字段-->
              <span v-else-if="item.formatter" class="inline-span" :title="item.formatter(row[item.prop])">
                {{ item.formatter(row[item.prop]) }}
              </span>
              <span v-else class="inline-span" :title="row[item.prop]">
                {{ row[item.prop] }}
              </span>
            </slot>
          </template>
